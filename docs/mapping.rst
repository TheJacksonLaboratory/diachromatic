...
corresponds to the **average size of fragments of the Hi-C library**. The size distribution of un-ligated fragments
should be the same as for hybrid fragments. However, the determination of this distribution is not straightforward.
For hybrid fragments the size is calculated as the sum of the two distances between the 5' ends of the mapped reads and
the next occurrence of a cutting motif in 3' direction which is assumed to correspond to the ligation junction
(Wingett 2015). The problem with this approach is that in fact the ligation junction cannot be unambiguously determined,
because the digestion of genome is not necessarily complete, i.e. there may be restriction fragments containing uncut
restriction sites (in the illustration marked with asterisk). In such cases, the size is underestimated, because, for
lack of further information, simply the first occurrence of a cutting motif is interpreted as the one that corresponds
to the ligation junction. Therefore, Diachromatic does not use this approach but requires this parameter to be specified
using the ``-l <size>`` option. We recommend to use external tool such as the `peak caller Q`_ for fragment size
...
observed for ChIP-seq reads. Within Diachromatic, inward pointing read pairs for which the distance between the 5' ends
is less than the specified threshold are categorized as un-ligated pairs.
...
Furthermore, self-ligating fragments have a different size distribution than hybrid or un-ligated fragments.

Therefore, the relevant size is no longer the size of the sequenced fragment that results from sonication but the
favourable size at which fragments self-ligate.

Very short fragments might not self-ligate because of steric hindrance, whereas the ends of very long fragments might
be unlikely to become located in sufficient physical proximity.

This size has too be calculated differently for given read pairs.

Should be the size of the distance between the 5' end positions d plus the calculated isert size d' (see illustration
below).
